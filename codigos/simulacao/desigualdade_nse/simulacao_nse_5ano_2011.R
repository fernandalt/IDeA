...
for(i in 1:nrow(codigos_municipios)){
  ...
  cod_escolas = dados_tudo[ID_MUNICIPIO==codigos_municipios[i,ID_MUNICIPIO] &  ID_SERIE==5 &
                             !is.na(Leitura) & !is.na(Matematica) & classificacao_NSE %in% c(1,5) & Ano %in% c(2007,2009,2011),
...
save(amostras_simuladas, codigos_municipios, tamanho_amostra, 
     file = paste0(path, '/simulacao/desigualdade_nse/simulacao_nse_5ano_lp_2011.Rdata'))
...
save(amostras_simuladas, codigos_municipios, tamanho_amostra,
     file = paste0(path, '/simulacao/desigualdade_nse/simulacao_nse_5ano_mat_2011.Rdata'))