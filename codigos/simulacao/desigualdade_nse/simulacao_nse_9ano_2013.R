...
tamanho_amostra = dados_tudo[!is.na(Leitura) & !is.na(Matematica) & classificacao_NSE %in% c(1,5) & Ano %in% c(2009,2011,2013)
...
for(i in 1:nrow(codigos_municipios)){
  ...
  cod_escolas = dados_tudo[ID_MUNICIPIO==codigos_municipios[i,ID_MUNICIPIO] &  ID_SERIE == 9 & !is.na(Matematica) &
                            !is.na(Leitura) & classificacao_NSE %in% c(1,5) & Ano %in% c(2009,2011,2013),
...
save(amostras_simuladas, codigos_municipios, tamanho_amostra, 
     file = paste0(path, '/simulacao/desigualdade_nse/simulacao_nse_9ano_lp_2013.Rdata'))
...
for(i in 1:nrow(codigos_municipios)){
  ...
  cod_escolas = dados_tudo[ID_MUNICIPIO==codigos_municipios[i,ID_MUNICIPIO] &  ID_SERIE == 9 & !is.na(Leitura) &
                             !is.na(Matematica) & classificacao_NSE %in% c(1,5) & Ano %in% c(2009,2011,2013),
                           .N,.(PK_COD_ENTIDADE,classificacao_NSE)]
...
save(amostras_simuladas, codigos_municipios, tamanho_amostra, 
     file = paste0(path, '/simulacao/desigualdade_nse/simulacao_nse_9ano_mat_2013.Rdata'))