...
prop_nse_escola = prop_nse_escola_11.15_5ano
...
prop_nse_municipio = prop_nse_municipio_11.15_5ano
...
tamanho_amostra = dados_tudo[!is.na(Leitura) & !is.na(Matematica) & classificacao_NSE %in% c(1,5) & Ano %in% c(2011,2013,2015)
...
codigos_municipios = tamanho_amostra[ID_SERIE == 5 & (NSE1 <30 | NSE5 < 30),
...
load(paste0(path,'/modelos/modelo_5ano_lp_2015.Rdata'))

sd_intercepto = VarCorr(modelo.Completo.5ano.lp.11.15)[[24]]
sd_intercepto = as.numeric(sd_intercepto)

efeito_aleatorio_final_Completo = read.csv2(paste0(path,'/correcao_nse/coeficientes_5Ano_lp_2015.csv'))
...
for(i in 1:nrow(codigos_municipios)){
  ...
  cod_escolas = dados_tudo[ID_MUNICIPIO==codigos_municipios[i,ID_MUNICIPIO] &  ID_SERIE==5 & 
                            !is.na(Leitura) & !is.na(Matematica) & classificacao_NSE %in% c(1,5) & Ano %in% c(2011,2013,2015),
...
save(amostras_simuladas, codigos_municipios, tamanho_amostra, 
     file = paste0(path, '/simulacao/desigualdade_nse/simulacao_nse_5ano_lp_2015.Rdata'))

# Dados modelo MAT
load(paste0(path,'/modelos/modelo_5ano_mat_2015.Rdata'))

sd_intercepto = VarCorr(modelo.Completo.5ano.mat.11.15)[[24]]
sd_intercepto = as.numeric(sd_intercepto)

efeito_aleatorio_final_Completo = read.csv2(paste0(path,'/correcao_nse/coeficientes_5Ano_mat_2015.csv'))
efeito_aleatorio_final_Completo = data.table(efeito_aleatorio_final_Completo)
...
for(i in 1:nrow(codigos_municipios)){
  ...
  cod_escolas = dados_tudo[ID_MUNICIPIO==codigos_municipios[i,ID_MUNICIPIO] &  ID_SERIE==5 & !is.na(Leitura) &
                             !is.na(Matematica) & classificacao_NSE %in% c(1,5) & Ano %in% c(2011,2013,2015),
                           .N,.(PK_COD_ENTIDADE,classificacao_NSE)]
...
save(amostras_simuladas, codigos_municipios, tamanho_amostra,
     file = paste0(path, '/simulacao/desigualdade_nse/simulacao_nse_5ano_mat_2015.Rdata'))