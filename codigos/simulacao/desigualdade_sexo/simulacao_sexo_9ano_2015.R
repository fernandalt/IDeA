...
codigos_municipios = tamanho_amostra[ID_SERIE == 9 & (Masculino <30 | Feminino < 30),
...
for(i in 1:nrow(codigos_municipios)){
  ...
  cod_escolas = dados_tudo[ID_MUNICIPIO==codigos_municipios[i,ID_MUNICIPIO] &  ID_SERIE==9 & 
                            !is.na(Leitura) & !is.na(Matematica) & !is.na(Sexo) & 
                             Ano %in% c(2011,2013,2015),
                          .N,.(PK_COD_ENTIDADE,Sexo)]
  
  alunos_ano = dados_tudo[ID_MUNICIPIO==codigos_municipios[i, ID_MUNICIPIO] &  ID_SERIE==9
                         & !is.na(Leitura) & !is.na(Matematica) & Ano %in% c(2011,2013,2015),.N,Ano]
...
save(amostras_simuladas, codigos_municipios, tamanho_amostra, 
     file = paste0(path, '/simulacao/desigualdade_sexo/simulacao_sexo_9ano_lp_2015.Rdata'))
...
save(amostras_simuladas, codigos_municipios, tamanho_amostra, 
     file = paste0(path, '/simulacao/desigualdade_sexo/simulacao_sexo_9ano_mat_2015.Rdata'))