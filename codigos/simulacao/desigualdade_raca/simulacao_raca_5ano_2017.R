...
prop_nse_escola = prop_nse_escola_13.17_5ano
...
prop_nse_municipio = prop_nse_municipio_13.17_5ano
...
tamanho_amostra = dados_tudo[!is.na(Leitura) & !is.na(Matematica) & Raça %in% c('Branco','Preto') & Ano %in% c(2013,2015,2017)
...
sd_intercepto = VarCorr(modelo.Completo.5ano.lp.13.17)[[24]]
sd_intercepto = as.numeric(sd_intercepto)

efeito_aleatorio_final_Completo = read.csv2(paste0(path,'/correcao_nse/coeficientes_5Ano_lp_2017.csv'))
...
for(i in 1:nrow(codigos_municipios)){
  ...
  cod_escolas = dados_tudo[ID_MUNICIPIO==codigos_municipios[i,ID_MUNICIPIO] &  ID_SERIE==5 & !is.na(Matematica) &
                             !is.na(Leitura) & Raça %in% c('Branco','Preto') & Ano %in% c(2013,2015,2017),
...
save(amostras_simuladas, codigos_municipios, tamanho_amostra, 
     file = paste0(path, '/simulacao/desigualdade_raca/simulacao_raca_5ano_lp_2017.Rdata'))

# Dados modelo MAT
load(paste0(path,'/modelos/modelo_5ano_mat_2017.Rdata'))

sd_intercepto = VarCorr(modelo.Completo.5ano.mat.13.17)[[24]]
sd_intercepto = as.numeric(sd_intercepto)

efeito_aleatorio_final_Completo = read.csv2(paste0(path,'/correcao_nse/coeficientes_5Ano_mat_2017.csv'))
efeito_aleatorio_final_Completo = data.table(efeito_aleatorio_final_Completo)
...
for(i in 1:nrow(codigos_municipios)){
  ...
  cod_escolas = dados_tudo[ID_MUNICIPIO==codigos_municipios[i,ID_MUNICIPIO] &  ID_SERIE==5 & !is.na(Leitura) & 
                             !is.na(Matematica) & Raça %in% c('Branco','Preto') & Ano %in% c(2013,2015,2017),
                           .N,.(PK_COD_ENTIDADE,Raça)]
...
save(amostras_simuladas, codigos_municipios, tamanho_amostra, 
     file = paste0(path, '/simulacao/desigualdade_raca/simulacao_raca_5ano_mat_2017.Rdata'))