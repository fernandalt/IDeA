...
retorna_valor_esperados = function(codigo_municipio,codigo_escola,cod_escolas,raca){
  ...
  if(nrow(dados_tudo[PK_COD_ENTIDADE==codigo_escola]) == 0){
    parcela_escola = unlist(efeito_aleatorio_final_Completo[COD_MUNICIPIO==codigo_municipio,'NSE_Escola']*mean(dados_tudo[ID_MUNICIPIO==codigo_municipio,NSE_Escola], na.rm=T))
  } else{
...
save(amostras_simuladas, codigos_municipios, tamanho_amostra, 
     file = paste0(path, '/simulacao/desigualdade_raca/simulacao_raca_5ano_lp_2011.Rdata'))
...
save(amostras_simuladas, codigos_municipios, tamanho_amostra, 
     file = paste0(path, '/simulacao/desigualdade_raca/simulacao_raca_5ano_mat_2011.Rdata'))