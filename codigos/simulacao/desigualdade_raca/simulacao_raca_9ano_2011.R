...
prop_nse_escola = prop_nse_escola_07.11_9ano
...
prop_nse_municipio = prop_nse_municipio_07.11_9ano
...
tamanho_amostra = dados_tudo[!is.na(Leitura) & !is.na(Matematica) & Raça %in% c('Branco','Preto') & Ano %in% c(2007,2009,2011)
...
retorna_valor_esperados = function(codigo_municipio,codigo_escola,cod_escolas,raca){
  ...
  if(nrow(dados_tudo[PK_COD_ENTIDADE==codigo_escola]) == 0){
    parcela_escola = unlist(efeito_aleatorio_final_Completo[COD_MUNICIPIO==codigo_municipio,'NSE_Escola']*mean(dados_tudo[ID_MUNICIPIO==codigo_municipio,NSE_Escola],na.rm=TRUE))
  } else{
...
load(paste0(path,'/modelos/modelo_9ano_lp_2011.Rdata'))

sd_intercepto = VarCorr(modelo.Completo.9ano.lp.07.11)[[24]]
sd_intercepto = as.numeric(sd_intercepto)

efeito_aleatorio_final_Completo = read.csv2(paste0(path,'/correcao_nse/coeficientes_9Ano_lp_2011.csv'))
...
for(i in 1:nrow(codigos_municipios)){
  ...
  cod_escolas = dados_tudo[ID_MUNICIPIO==codigos_municipios[i,ID_MUNICIPIO] &  ID_SERIE==9 & !is.na(Matematica) &
                             !is.na(Leitura) & Raça %in% c('Branco','Preto') & Ano %in% c(2007,2009,2011),
...
save(amostras_simuladas, codigos_municipios, tamanho_amostra, 
     file = paste0(path, '/simulacao/desigualdade_raca/simulacao_raca_9ano_lp_2011.Rdata'))

# Dados modelo MAT
load(paste0(path,'/modelos/modelo_9ano_mat_2011.Rdata'))

sd_intercepto = VarCorr(modelo.Completo.9ano.mat.07.11)[[24]]
sd_intercepto = as.numeric(sd_intercepto)

efeito_aleatorio_final_Completo = read.csv2(paste0(path,'/correcao_nse/coeficientes_9Ano_mat_2011.csv'))
efeito_aleatorio_final_Completo = data.table(efeito_aleatorio_final_Completo)
...
for(i in 1:nrow(codigos_municipios)){
  ...
  cod_escolas = dados_tudo[ID_MUNICIPIO==codigos_municipios[i,ID_MUNICIPIO] &  ID_SERIE==9 & !is.na(Leitura) &
                             !is.na(Matematica) & Raça %in% c('Branco','Preto') & Ano %in% c(2007,2009,2011),
                           .N,.(PK_COD_ENTIDADE,Raça)]
...
save(amostras_simuladas, codigos_municipios, tamanho_amostra, 
     file = paste0(path, '/simulacao/desigualdade_raca/simulacao_raca_9ano_mat_2011.Rdata'))