...
tamanho_amostra = dados_tudo[!is.na(Leitura) & !is.na(Matematica) & Raça %in% c('Branco','Preto') & Ano %in% c(2011,2013,2015)
...
codigos_municipios = tamanho_amostra[ID_SERIE == 9 & (Branco <30 | Preto <30),
...
for(i in 1:nrow(codigos_municipios)){
  ...
  cod_escolas = dados_tudo[ID_MUNICIPIO==codigos_municipios[i,ID_MUNICIPIO] &  ID_SERIE==9 & !is.na(Matematica) & 
                             !is.na(Leitura) & Raça %in% c('Branco','Preto') & Ano %in% c(2011,2013,2015),
...
save(amostras_simuladas, codigos_municipios, tamanho_amostra, 
     file = paste0(path, '/simulacao/desigualdade_raca/simulacao_raca_9ano_lp_2015.Rdata'))
...
for(i in 1:nrow(codigos_municipios)){
  ...
  cod_escolas = dados_tudo[ID_MUNICIPIO==codigos_municipios[i,ID_MUNICIPIO] &  ID_SERIE==9 & !is.na(Leitura) &
                             !is.na(Matematica) & Raça %in% c('Branco','Preto') & Ano %in% c(2011,2013,2015),
                           .N,.(PK_COD_ENTIDADE,Raça)]
...
save(amostras_simuladas, codigos_municipios, tamanho_amostra, 
     file = paste0(path, '/simulacao/desigualdade_raca/simulacao_raca_9ano_mat_2015.Rdata'))