...
prop_nse_escola = prop_nse_escola_09.13_9ano
...
prop_nse_municipio = prop_nse_municipio_09.13_9ano
...
sd_intercepto = VarCorr(modelo.Completo.9ano.lp.09.13)[[24]]
sd_intercepto = as.numeric(sd_intercepto)

efeito_aleatorio_final_Completo = read.csv2(paste0(path,'/correcao_nse/coeficientes_9Ano_lp_2013.csv'))
...
for(i in 1:length(codigos_municipios)){
  ...
  cod_escolas = dados_tudo[ID_MUNICIPIO==codigos_municipios[i] &  ID_SERIE==9 & !is.na(Leitura) & !is.na(Matematica)
                          & Ano %in% c(2009,2011,2013),.N,PK_COD_ENTIDADE]
...
save(amostras_simuladas35, medias_perfis35,codigos_municipios, 
     file = paste0(path, '/simulacao/nivel/simulacao_nivel_9ano_lp_2013.Rdata'))

# Dados modelo MAT
load(paste0(path,'/modelos/modelo_9ano_mat_2013.Rdata'))

sd_intercepto = VarCorr(modelo.Completo.9ano.mat.09.13)[[24]]
sd_intercepto = as.numeric(sd_intercepto)

efeito_aleatorio_final_Completo = read.csv2(paste0(path,'/correcao_nse/coeficientes_9Ano_mat_2013.csv'))
efeito_aleatorio_final_Completo = data.table(efeito_aleatorio_final_Completo)
...
save(amostras_simuladas35, medias_perfis35, codigos_municipios, 
     file = paste0(path, '/simulacao/nivel/simulacao_nivel_9ano_mat_2013.Rdata'))