...
prop_nse_escola = prop_nse_escola_11.15_9ano
...
prop_nse_municipio = prop_nse_municipio_11.15_9ano
...
# Identifica municipios com numero de alunos insuficiente e define valor de referencia
tamanho_amostra = dados_tudo[!is.na(Leitura) & !is.na(Matematica) & Ano %in% c(2011,2013,2015)
                             ,.N, .(ID_MUNICIPIO, ID_SERIE)]

codigos_municipios = tamanho_amostra[N<100 & ID_SERIE==9,ID_MUNICIPIO]

n_sim = quantile(tamanho_amostra[N<100 & ID_SERIE==9,N],0.25) - min(tamanho_amostra[N<100 & ID_SERIE==9,N])
n_sim = ceiling(n_sim)
...
retorna_valor_esperados = function(codigo_municipio,codigo_escola){
  ...
  parcela_ano = c('2011' = 0,unlist(efeito_aleatorio_final_Completo[COD_MUNICIPIO==codigo_municipio,
                                                                   which(colnames(efeito_aleatorio_final_Completo) %in% 
                                                                           c("as.factor.Ano.2013","as.factor.Ano.2015")),with=F]))[perfil_ano]
...
load(paste0(path,'/modelos/modelo_9ano_lp_2015.Rdata'))

sd_intercepto = VarCorr(modelo.Completo.9ano.lp.11.15)[[24]]
sd_intercepto = as.numeric(sd_intercepto)

efeito_aleatorio_final_Completo = read.csv2(paste0(path,'/correcao_nse/coeficientes_9Ano_lp_2015.csv'))
...
for(i in 1:length(codigos_municipios)){
  ...
  cod_escolas = dados_tudo[ID_MUNICIPIO==codigos_municipios[i] &  ID_SERIE==9 & !is.na(Leitura) & !is.na(Matematica)
                          & Ano %in% c(2011,2013,2015),.N,PK_COD_ENTIDADE]
  ...
  alunos_ano = dados_tudo[ID_MUNICIPIO==codigos_municipios[i] &  ID_SERIE==9
                         & !is.na(Leitura) & Ano %in% c(2011,2013,2015),.N,Ano]
  
  alunos_ano$prop = alunos_ano$N/sum(alunos_ano$N)
  
  if(nrow(alunos_ano)<3){
    ind = which(is.na(match(c(2011,2013,2015),alunos_ano$Ano)))
    alunos_ano = rbind(alunos_ano,data.table(Ano=c(2011,2013,2015)[ind],N=0,prop=0))
  }
...
save(amostras_simuladas35, medias_perfis35,codigos_municipios, 
     file = paste0(path, '/simulacao/nivel/simulacao_nivel_9ano_lp_2015.Rdata'))

# Dados modelo MAT
load(paste0(path,'/modelos/modelo_9ano_mat_2015.Rdata'))

sd_intercepto = VarCorr(modelo.Completo.9ano.mat.11.15)[[24]]
sd_intercepto = as.numeric(sd_intercepto)

efeito_aleatorio_final_Completo = read.csv2(paste0(path,'/correcao_nse/coeficientes_9Ano_mat_2015.csv'))
efeito_aleatorio_final_Completo = data.table(efeito_aleatorio_final_Completo)
...
for(i in 1:length(codigos_municipios)){
  ...
  alunos_ano = dados_tudo[ID_MUNICIPIO==codigos_municipios[i] &  ID_SERIE==9
                         & !is.na(Matematica) & Ano %in% c(2011,2013,2015),.N,Ano]
...
save(amostras_simuladas35, medias_perfis35, codigos_municipios,
     file = paste0(path, '/simulacao/nivel/simulacao_nivel_9ano_mat_2015.Rdata'))